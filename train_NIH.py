...
if __name__ == "__main__":
    ...
    for epoch_num in tqdm(range(max_epoch), ncols=70):
        ...
        for i_batch, sampled_batch in enumerate(trainloader):
            ...
            if iter_num % 5000 == 0:
                ...
                for param_group in l_optimizer.param_groups:
                    param_group['lr'] = lr_
            if iter_num % 1000 == 0:
                r_save_mode_path = os.path.join(snapshot_path, 'nihr_model_iter_' + str(iter_num) + '.pth')
                l_save_mode_path = os.path.join(snapshot_path, 'nihl_model_iter_' + str(iter_num) + '.pth')
        ...
        if iter_num >= max_iterations:
            break
    r_model_save_mode_path = os.path.join(snapshot_path, '256nihr_model_iter_'+str(max_iterations)+'lr'+str(base_lr)+'.pth')
    l_model_save_mode_path = os.path.join(snapshot_path, '256nihl_model_iter_'+str(max_iterations)+'lr'+str(base_lr)+'.pth')
