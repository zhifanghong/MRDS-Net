...
if __name__ == "__main__":
    ...
    labeled_idxs = list(range(35))
    unlabeled_idxs = list(range(36, 178))
    ...
    for epoch_num in tqdm(range(max_epoch), ncols=70):
        ...
        for i_batch, sampled_batch in enumerate(trainloader):
            ...
            #preds = torch.zeros([stride * T, 2, 112, 112, 80]).cuda()   添加大小为（112，112，80）的crop
            preds = torch.zeros([stride * T, 2, 208, 208, 80]).cuda()  #不添加crop
            for i in range(T//2):
                ema_inputs = volume_batch_r + torch.clamp(torch.randn_like(volume_batch_r) * 0.1, -0.2, 0.2)
                with torch.no_grad():
                    preds[2 * stride * i:2 * stride * (i + 1)] = ema_model(ema_inputs)
            preds = F.softmax(preds, dim=1)
            #preds = preds.reshape(T, stride, 2, 112, 112, 80)  添加大小为（112，112，80）的crop
            preds = preds.reshape(T, stride, 2, 208, 208, 80)  #不添加crop
            ...
            logging.info('iteration %d : loss : %f cons_dist: %f, loss_weight: %f , dice_loss:%f' %
                         (iter_num, loss.item(), consistency_dist.item(), consistency_weight , loss_seg_dice.item()))
