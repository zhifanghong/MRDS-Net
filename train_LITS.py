...
#######
#######   train.list的1-149为有标签数据  150-799为无标签数据
#######
import torch
...
if __name__ == "__main__":
    ...
    for epoch_num in tqdm(range(max_epoch), ncols=70):
        ...
        for i_batch, sampled_batch in enumerate(trainloader):
            ...
            if iter_num % 100 == 0:
                image = volume_batch[0, 0:1, :, :, 10:41:5].permute(3, 0, 1, 2).repeat(1, 3, 1, 1)
                grid_image = make_grid(image, 5, normalize=True)
                writer.add_image('train/Image', grid_image, iter_num)

                # image = outputs_soft[0, 3:4, :, :, 20:61:10].permute(3, 0, 1, 2).repeat(1, 3, 1, 1)
                image = torch.max(l_outputs_soft[0, :, :, :, 10:41:5], 0)[1].permute(2, 0, 1).data.cpu().numpy()
                image = utils.decode_seg_map_sequence(image)
                grid_image = make_grid(image, 5, normalize=False)
                writer.add_image('train/Predicted_label', grid_image, iter_num)

                image = label_batch[0, :, :, 10:41:5].permute(2, 0, 1)
                grid_image = make_grid(utils.decode_seg_map_sequence(image.data.cpu().numpy()), 5, normalize=False)
                writer.add_image('train/Groundtruth_label', grid_image, iter_num)
            # change lr
            if iter_num % 4000 == 0:
                lr_ = base_lr * 0.1 ** (iter_num // 4000)
                for param_group in r_optimizer.param_groups:
                    param_group['lr'] = lr_
                for param_group in l_optimizer.param_groups:
                    param_group['lr'] = lr_
            if iter_num % 1000 == 0:
                r_save_mode_path = os.path.join(snapshot_path, 'litsr_model_iter_' + str(iter_num) + '.pth')
                l_save_mode_path = os.path.join(snapshot_path, 'litsl_model_iter_' + str(iter_num) + '.pth')
        ...
        if iter_num >= max_iterations:
            break
    r_model_save_mode_path = os.path.join(snapshot_path, '256LITSr_model_iter_'+str(max_iterations)+'lr'+str(base_lr)+'.pth')
    l_model_save_mode_path = os.path.join(snapshot_path, '256LITSl_model_iter_'+str(max_iterations)+'lr'+str(base_lr)+'.pth')
